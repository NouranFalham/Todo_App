...
function deleteTodo(id){
    ...
    const editedTodos = newTodos.filter(function(todo){
       return todo.id !==id //show all elements except el element ely el id bta3o mab3oot
    })
    ...
    return (
        ...
        <main>
            <div className="content">
                <div className="infoo d-flex justify-content-center gap-5">
                    <span> <i class="fa-solid fa-thumbtack-slash"></i>Remaining Tasks: {remainingTasks}</span>
                    <span> <i class="fa-solid fa-thumbtack"></i>Complete Tasks: {completedTasks}</span>
                </div>
                <div className="buttons align-items-center justify-content-center d-flex gap-2 my-3">
                    <button onClick={completeAll} className="btn btn-danger w-50 ">Complete All</button>
                    <button onClick={clearCompleted} className="btn btn-danger w-50 ">Clear Completed </button>
                </div>
                <ul>
                    {todos.map(function(todo){
                        return <Todo key={todo.id} todoinfo= {todo.todo} isCompleted={todo.completed} deleteTodo={function(){
                            deleteTodo(todo.id)
                        }} 
